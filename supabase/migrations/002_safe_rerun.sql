-- ============================================================================
-- DispaLoadIQ — Safe Re-run Migration (skips existing objects)
-- Paste this in Supabase: SQL Editor → New query → Run
-- ============================================================================

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ── ENUMS (safe — wrapped in DO blocks) ──────────────────────────────────────

DO $$ BEGIN CREATE TYPE user_role AS ENUM ('owner-op', 'dispatcher', 'company', 'shipper'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE load_status AS ENUM ('posted', 'bidding', 'booked', 'dispatched', 'in_transit', 'delivered', 'cancelled'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE trip_status AS ENUM ('scheduled', 'in_transit', 'delivered', 'issue'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE claim_status AS ENUM ('open', 'disputed', 'paid', 'denied'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE claim_type AS ENUM ('damage', 'shortage', 'delay', 'theft', 'contamination'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE invoice_status AS ENUM ('draft', 'sent', 'paid', 'overdue', 'disputed'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE contract_status AS ENUM ('draft', 'pending_sign', 'active', 'expired', 'terminated'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE fuel_type AS ENUM ('diesel', 'def', 'gasoline'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE maintenance_status AS ENUM ('scheduled', 'in_progress', 'completed', 'cancelled'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE contract_type AS ENUM ('dispatcher_agreement', 'carrier_agreement', 'broker_agreement', 'other'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS dispatcher_clients (
  id               UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
DROP TRIGGER IF EXISTS trg_bid_count ON load_bids;
CREATE TRIGGER trg_bid_count
  AFTER INSERT OR DELETE ON load_bids FOR EACH ROW EXECUTE FUNCTION increment_bids_count();
...
SELECT l.*, p.full_name AS shipper_name, p.mc_number AS shipper_mc, p.company_name AS shipper_company
FROM loads l JOIN user_profiles p ON p.id = l.shipper_id;

CREATE OR REPLACE FUNCTION get_lane_stats(p_origin_state TEXT, p_destination_state TEXT)
RETURNS TABLE(avg_rate NUMERIC, avg_rpm NUMERIC, trip_count BIGINT, avg_miles NUMERIC) AS $$
BEGIN
  RETURN QUERY
  SELECT ROUND(AVG(rate), 2), ROUND(AVG(rate / NULLIF(miles, 0)), 3), COUNT(*), ROUND(AVG(miles), 0)
  FROM loads
  WHERE origin_state = p_origin_state AND destination_state = p_destination_state
    AND status IN ('delivered', 'booked') AND rate IS NOT NULL AND miles IS NOT NULL;
END;
$$ LANGUAGE plpgsql;
...
-- trips
CREATE POLICY "Trip participants can view"   ON trips FOR SELECT USING (auth.uid() IN (driver_id, dispatcher_id, company_id));
CREATE POLICY "Trip participants can insert" ON trips FOR INSERT WITH CHECK (auth.uid() IN (driver_id, dispatcher_id, company_id));
CREATE POLICY "Trip participants can update" ON trips FOR UPDATE USING (auth.uid() IN (driver_id, dispatcher_id, company_id));
...
CREATE POLICY "DC participants"     ON dispatcher_clients  FOR SELECT USING (auth.uid() IN (dispatcher_id, carrier_id));
CREATE POLICY "Dispatchers create DC" ON dispatcher_clients FOR INSERT WITH CHECK (auth.uid() = dispatcher_id);
CREATE POLICY "Claimant photo access" ON claim_photos FOR ALL USING (auth.uid() IN (SELECT claimant_id FROM claims WHERE id = claim_id));
